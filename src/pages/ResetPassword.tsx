      ...
      <div className="flex flex-col items-center justify-center flex-grow text-center px-2 sm:px-4 md:px-0">
        {!success ? (
          <>
            ...
            <h2 className="text-xl font-[lufga] sm:text-2xl md:text-4xl font-bold text-[#090933] mb-2">Définir un nouveau mot de passe</h2>
            <p className="text-xs font-[lufga] sm:text-sm md:text-base text-[#7C829C] mb-6 sm:mb-8">Entrez votre nouveau mot de passe ci-dessous</p>
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-[320px] sm:max-w-md lg:max-w-lg space-y-3 sm:space-y-4 md:space-y-6">
              ...
              <button
                ...
                className={`w-full py-2 sm:py-3 md:py-4 rounded-lg text-sm sm:text-base md:text-lg transition-all ${
                  isValidPassword(password) && password === confirmPassword
                    ? "bg-[#193AF9] font-[lufga] text-white hover:bg-gray-800"
                    : "bg-[#7C829C] font-[lufga] text-white cursor-not-allowed"
                }`}
              ...
              <div className="bg-red-50 rounded-lg p-2 sm:p-3 md:p-4 shadow-sm w-full flex flex-wrap gap-y-2">
                <div className="w-full sm:w-1/2 flex items-center gap-1 sm:gap-2">
                  <span className={password.length >= 8 ? "text-[#028361]" : "text-[#193AF9]"}>✗</span>
                  <span className={`${password.length >= 8 ? "text-[#028361]" : "text-[#193AF9]"} text-xs sm:text-sm`}>Minimum 8 caractères</span>
                </div>

                <div className="w-full sm:w-1/2 flex items-center gap-1 sm:gap-2">
                  <span className={/[A-Z]/.test(password) ? "text-[#028361]" : "text-[#FC5185]"}>✗</span>
                  <span className={`${/[A-Z]/.test(password) ? "text-[#028361]" : "text-[#FC5185]"} text-xs sm:text-sm`}>Au moins 1 MAJUSCULE</span>
                </div>

                <div className="w-full sm:w-1/2 flex items-center gap-1 sm:gap-2">
                  <span className={/\d/.test(password) ? "text-[#028361]" : "text-[#193AF9]"}>✗</span>
                  <span className={`${/\d/.test(password) ? "text-[#028361]" : "text-[#193AF9]"} text-xs sm:text-sm`}>Au moins 1 chiffre</span>
                </div>

                <div className="w-full sm:w-1/2 flex items-center gap-1 sm:gap-2">
                  <span className={/[!@#$%^&*]/.test(password) ? "text-[#028361]" : "text-[#FC5185]"}>✗</span>
                  <span className={`${/[!@#$%^&*]/.test(password) ? "text-[#028361]" : "text-[#FC5185]"} text-xs sm:text-sm`}>Au moins 1 symbole</span>
                </div>
              </div>

              {error && <p className="text-[#193AF9] mt-3 sm:mt-4 text-xs sm:text-sm">{error}</p>}
            </form>
          </>
        ) : (
          <>
            <img src="src/assets/images/icon-logo.svg" alt="Logo Icon" className="w-10 h-12 sm:w-12 sm:h-14 md:w-14 mb-3 sm:mb-4 md:mb-6" />
            <h2 className="text-xl sm:text-2xl md:text-4xl font-bold text-[#090933] mb-2">Mot de passe réinitialisé</h2>
            <p className="text-xs font-[lufga] sm:text-sm md:text-base text-[#7C829C] mb-4 sm:mb-6 max-w-[280px] sm:max-w-sm md:max-w-lg px-2 sm:px-4">
            ...
            <button className="w-full max-w-[280px] sm:max-w-xs md:max-w-md py-2 sm:py-3 rounded-lg bg-[#193AF9] text-white text-sm sm:text-base md:text-lg hover:bg-[#193AF9] transition-all flex items-center justify-center gap-2">
