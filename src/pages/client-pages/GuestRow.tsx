...
export const GuestRow: React.FC<GuestRowProps> = ({ guest, isSelected, onSelect }) => {
  ...
  return (
    <div className={`flex flex-col justify-between px-5 py-2  mt-2.5 w-full max-w-[1294px] h-[50px] max-md:max-w-full ${isSelected ? 'bg-indigo-400 bg-opacity-10 rounded-xl' : ''}`}>
      <div className="flex flex-wrap gap-3.5 justify-between w-full max-md:max-w-full">
        <div className="flex justify-center items-center h-full w-[70px]">
          <div
          ...
          >
            {isSelected && <div className="flex my-auto w-3 bg-blue-700 rounded-[1.4px] h-[12px]" />}
          </div>
        </div>
        <div className="flex gap-2 items-center  text-sm font-medium h-[30px] text-slate-500 w-[230px]">
          ...
          />
          <div className="font-lufga font-medium my-auto w-[126px]">{guest.name}</div>
        </div>
        <div className="font-lufga font-regular text-center text-slate-500 w-[315px]">{guest.email}</div>
        <div className="font-lufga font-regular text-center text-slate-500 w-[163px]">{guest.sentDate}</div>
        <div className="font-lufga font-regular text-center text-slate-500 w-[196px]">{guest.expirationDate}</div>
        ...
        {open &&
          <div className='flex flex-col bg-[#FFFFFF] shadow-lg rounded-[10px] px-[20px] py-[10px] space-y-[10px]'>
        <button className='flex '>  
          ...
          <button className='flex '><svg width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
